...
一个可以直接点开的本地 Web MVP，把 skill 的方法论跑给人看。它补上 CLI 流水线**故意没有自动化**的两件事：

1. **看得见的趋势获取** —— 每个词按序走降级链（sellersprite → sorftime → 人工粘贴），
   记录每次尝试的层级、耗时、原始响应与失败原因，命中即停。
   每个数字都能点开看到它从哪个 tier 来。
2. **必须由人回答的问题** —— 每个阶段有一个机器不该替你回答的问题。
...
这几步的决策框是手填 —— 目的是先把**交互形态**和**趋势获取**立起来，
而不是假装语料聚类已经自动化了。
