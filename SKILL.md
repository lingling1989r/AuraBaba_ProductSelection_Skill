...
| 输入 | 谁给 | 为什么不能猜 |
|---|---|---|
| **调研名称**（`--topic`） | 用户 | 这一轮选品的**记录名**，写进 `state.json` / Excel「主题」行 / 报告标题。**不参与取数**，但没有它这一轮记录就没法被找回来 |
...
三样都是**用户输入**。用户只说「帮我选品」时，一次问齐：

> 1. 这次调研叫什么名字？（方便之后查找记录）
> 2. 做哪个市场（US / UK / DE / JP…）？
> 3. 给我 2–3 个你已知的**圈层身份词**做种子（例如 `that girl`、`pilates girl`）
>    —— 不要给商品词，商品词是后面才出现的东西。

`init` 对 market / seed 缺失会直接报错退出（exit 2），不会带着空种子往下跑。
名称虽然也必填，但要提醒用户：**它只是标识，改名字不会改变任何数据**。
种子进了 `state.json` 之后，S1 再往 `data/01_seed_keywords.csv` 里补新发现的身份词，
S2 取的是**两者并集**。
